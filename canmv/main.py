...
# 3) reporting cut_request over UART/USB JSON and optional GPIO hard trigger
# Motor/feed/cutter execution is handled on the Raspberry Pi side.
TRIGGER_ENABLE = True
TRIGGER_PIN = 32                 # Set this to the CanMV GPIO output pin wired to Raspberry Pi
...
# UART/USB output (AI detections + cut decision/state)
            if TRANSPORT_SEND_ENABLE and _ticks_diff(now_ms, _transport_last_ms) >= TRANSPORT_SEND_MIN_INTERVAL_MS:
                payload = {
                    ...
                    # Keep UART as a status/diagnostic channel. The Raspberry Pi
                    # may use GPIO as the primary hard trigger, but it still
                    # benefits from seeing the current cut_request state here.
                    "cut_request": bool(trigger_active),
                    "cut_request_edge": bool(trigger_rise),
