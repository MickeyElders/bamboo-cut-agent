...
DKC_SERIAL_PORT=/dev/ttyUSB0
DKC_BAUDRATE=9600
DKC_SLAVE_ID=1
DKC_WORD_ORDER=0
DKC_POSITION_SCALE=1000
DKC_AXIS_POSITION_D=1
DKC_AXIS_SPEED_FEEDBACK_D=11
DKC_CUTTER_DOWN_TARGET_D=30
DKC_CUTTER_DOWN_SPEED_D=31
DKC_CUTTER_UP_TARGET_D=32
DKC_CUTTER_UP_SPEED_D=33
DKC_CUTTER_ZERO_TARGET_D=34
DKC_CUTTER_DOWN_SPEED=10000
DKC_CUTTER_UP_SPEED=10000
DKC_CUTTER_DOWN_M=1
DKC_CUTTER_UP_M=2
DKC_CUTTER_ZERO_M=3
DKC_CUTTER_BUSY_M=10
DKC_CUTTER_DONE_M=11
DKC_CUTTER_FAULT_M=12
```
...
### DKC-Y3X0 RS485 Control

如果已经切到 `DKC-Y320 / DKC-Y360` 这类控制器，后端现在支持通过 `RS485 Modbus RTU` 触发刀轴动作：

- `Raspberry Pi USB-RS485 A` -> `DKC 485A / D+`
- `Raspberry Pi USB-RS485 B` -> `DKC 485B / D-`
- `Raspberry Pi GND` -> `DKC GND`（推荐共地）

当前后端的实现方式：

- 通过 `.prt` 已确认 1 轴反馈默认绑定 `D1`，速度反馈默认绑定 `D11`
- 树莓派通过 Modbus 写 `D30-D34` 下发目标和设零参数
- 树莓派通过 Modbus 写 `M1-M3` 触发“下压 / 上升 / 设零”程序
- 树莓派通过 `D1` 回读当前真实位置，通过 `M10-M12` 读取忙碌 / 完成 / 故障状态

说明：

- 这是正式链路，优先级高于 GPIO 点脉冲原型链路
- 当前 UI 的“切刀下压 / 切刀抬起”会直接走这条 Modbus 链路
- `DKC_POSITION_SCALE=1000` 表示 1.000 mm 会按 `1000` 写入 D 寄存器，如果你的 DKC 工程按整数毫米处理，可改成 `1`
